...
// add event listener to check if DOM content is loaded tthen run todos on localStorage
...
function actionsOnToDo(e) {
  ...
  if(item.classList[0] === 'edit-btn') {
   ...
   // new input for editing
    ...
    // new button to add edited value
    const newInputButton = document.createElement("button");
    newInputButton.classList.add('new-input-button');
    newInputButton.innerHTML = `<i class="fas fa-plus-square"></i>`;
    displayTodos.appendChild(newInputButton)
    
    const todoItem = document.querySelector('.todo-item');
     const todoInnerText = todoItem.innerText;
    newIput.value = todoInnerText;
    todoList.removeChild(todoItem.parentElement)
...
// display todos on localStorage
function getTodos() {
  let storage = JSON.parse(localStorage.getItem('storage')) || [];
 
  //todos in the storage are looped over
  storage.forEach(function(todo) {
  // then the add todo function is recreated
